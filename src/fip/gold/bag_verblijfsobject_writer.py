...
from fip.lakehouse.silver.bag_verblijfsobject import BAG_VERBLIJFSOBJECT_FIELDS
from fip.settings import get_settings


class BAGVerblijfsobjectLandingWriter:
    ...
    def _insert_rows(
        ...
        schema_name = get_settings().postgres_schema
        columns = SQL(", ").join(Identifier(field) for field in BAG_VERBLIJFSOBJECT_FIELDS)
        placeholders = SQL(", ").join(SQL("%s") for _ in BAG_VERBLIJFSOBJECT_FIELDS)
        ...
        with conn.cursor() as cur:
            cur.executemany(
                sql,
                [tuple(row[field] for field in BAG_VERBLIJFSOBJECT_FIELDS) for row in rows],
            )
