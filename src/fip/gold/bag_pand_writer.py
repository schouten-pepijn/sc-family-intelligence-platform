...
from fip.lakehouse.silver.pdok_bag.bag_pand import BAG_PAND_FIELDS


class BAGPandLandingWriter(PostgresFullRefreshWriter):
    """Writes BAG pand rows to Postgres in the landing layer."""

    def _to_row(self, row: object) -> dict[str, object]:
        mapping = row
        return {field: mapping[field] for field in BAG_PAND_FIELDS}  # type: ignore[index]

    def _field_names(self) -> Sequence[str]:
        return BAG_PAND_FIELDS
