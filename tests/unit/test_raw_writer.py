...
def test_raw_snapshot_writer_writes_bag_gpkg_jsonl(tmp_path: Path) -> None:
    writer = RawSnapshotWriter(base_dir=tmp_path)
    records = [
        make_record(
            "bag.verblijfsobject",
    ...
    assert written == 1
    path = tmp_path / "raw" / "bag_gpkg" / "debug-raw" / "verblijfsobject.jsonl"
    assert path.exists()
    assert path.read_text(encoding="utf-8").count("\n") == 1


def test_s3_raw_snapshot_writer_writes_bag_gpkg_jsonl(monkeypatch) -> None:
    class FakeFile:
        def __init__(self, buffer: io.StringIO) -> None:
            self.buffer = buffer
    ...
    assert written == 1
    path = "s3://fip-lakehouse/raw/bag_gpkg/debug-raw/verblijfsobject.jsonl"
    assert writer.filesystem.paths == [path]
    content = writer.filesystem.buffers[path].getvalue()
    assert content.startswith('{"source_name": "bag_gpkg"')
    assert content.count("\n") == 1
